...
{
	private $title = 'Đối tác';
	private $template = 'backend/partner/';
	var	$path_url = 'cpanel/partner/';
	var $path_dir = 'uploads/images/partner/';
	var $path_dir_webp = 'uploads/webps/partner/';
	var $control = 'partner';
	function __construct()
	{
		$this->PartnerModels = new PartnerModels();
	...
	public function index()
	{
		...
		$datas = $this->PartnerModels->select_array('id,name,thumb,created_at,publish,sort', NULL, 'sort asc,id desc');
		$data = [
	...
	function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max = $this->PartnerModels->select_max('sort');
		if ($this->request->getPost()) {
			...
			$result = $this->PartnerModels->add($data_post);
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas 		= $this->PartnerModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->PartnerModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$datas = $this->PartnerModels->where_in(array('id' => $id));
		...
		if ($datas != NULL) {
			...
			$result = $this->PartnerModels->deleteWhere(array('id' => $id));
		} else {
			$result = $this->PartnerModels->deleteWhere(array('id' => $id));
		}
	}
	function deleteAll()
	{
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->PartnerModels->where_in(array('id' => $val));
			$filename = $this->path_dir . $datas['image'];
			$filenamethumb = $this->path_dir . $datas['thumb'];
			// webp
			if ($datas != NULL) {
				...
				$result = $this->PartnerModels->deleteWhere(array('id' => $val));
				if ($result) {
	...
	function checkglobals()
	{
		...
		$result = $this->PartnerModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	public function sort()
	{
		...
		$result = $this->PartnerModels->edit($data_update, array('id' => $id));
		if ($result > 0) {
