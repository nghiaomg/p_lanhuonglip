...
use App\Models\MethodPayModels;
use App\Models\AliasModels;
use App\Models\Upload;
use App\Models\CI_function;

class Methodpay extends BaseController
{
	private $title = 'Phương thức thanh toán';
	private $template = 'backend/methodpay/';
	var	$path_url = 'cpanel/methodpay/';
	var $path_dir = 'uploads/images/methodpay/';
	var $path_dir_webp = 'uploads/webps/methodpay/';
	var $control = 'methodpay';
	function __construct()
	{
		$this->MethodPayModels = new MethodPayModels();
	...
	public function index()
	{
		...
		$datas = $this->MethodPayModels->select_array('id,name,thumb,created_at,publish,sort', NULL, 'sort asc,id desc');
		$data = [
	...
	function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max = $this->MethodPayModels->select_max('sort');
		if ($this->request->getPost()) {
			...
			if ($_FILES['image']['name']) {
				...
				$data_upload = $this->Upload->upload_image($path_dir, $fileName, uniqid().'-'.$alias, 'image', 288, 575, null, null);
			...
			$result = $this->MethodPayModels->add($data_post);
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas 		= $this->MethodPayModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			if ($_FILES['image']['name']) {
				...
				$fileName = $this->request->getFile('image');
				$path_dir = $this->path_dir;
				$data_upload = $this->Upload->upload_image($path_dir, $fileName, uniqid().'-'.$_FILES['image']['name'], 'image', 288, 575, null, null);
				if ($data_upload['type'] == 'error') {
					return redirect()->to(base_url(CPANEL . $this->control . "/index"))->with('error', $data_upload['message']);
				} else {
					$image = date("Y") . '/' . date("m") . '/' . $data_upload['image'];
			...
			$result = $this->MethodPayModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$datas = $this->MethodPayModels->where_in(array('id' => $id));
		...
		if ($datas != NULL) {
			...
			$result = $this->MethodPayModels->deleteWhere(array('id' => $id));
		} else {
			$result = $this->MethodPayModels->deleteWhere(array('id' => $id));
		}
	}
	function deleteAll()
	{
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->MethodPayModels->where_in(array('id' => $val));
			$filename = $this->path_dir . $datas['image'];
			$filenamethumb = $this->path_dir . $datas['thumb'];
			// webp
			if ($datas != NULL) {
				...
				$result = $this->MethodPayModels->deleteWhere(array('id' => $val));
				if ($result) {
	...
	function checkglobals()
	{
		...
		$result = $this->MethodPayModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	public function sort()
	{
		...
		$result = $this->MethodPayModels->edit($data_update, array('id' => $id));
		if ($result > 0) {
