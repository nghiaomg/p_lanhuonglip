...
{
    private $title                  = 'chi nhánh';
    private $template               = 'backend/branch/';
    public $control                 = 'branch';
    public $path_url                = 'cpanel/branch/';
    ...
    public function index()
    {
        ...
        $datas      = $this->BranchModels->select_array("*", NULL, "sort asc, id desc");
        $data = array(
    ...
    function add()
    {
        ...
        $data_index = $this->get_indexBE();
        $sort_max   = $this->BranchModels->select_max('sort');
    ...
    function edit($id)
    {
        ...
        $data_index      = $this->get_indexBE();
        $datas           = $this->BranchModels->find_one($id);
        $sort_max        = $this->BranchModels->select_max('sort');
        if ($this->request->getPost()) {
            $data_post   = $this->request->getPost('data_post');
    ...
    function delete()
    {
        ...
        $this->checkPermission();
        $id     = $_POST['id'];
        $result = $this->BranchModels->deleteWhere(array('id' => $id));
    }
    ...
    function deleteAll()
    {
        ...
        foreach ($list_id_delete as $key => $val) {
            $result = $this->BranchModels->deleteWhere(array('id' => $val));
            if ($result) {
