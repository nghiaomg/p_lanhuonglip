...
{
	private $title = 'Video giới thiệu';
	private $template = 'backend/video/';
	var	$path_url = 'cpanel/video/';
	var $path_dir = 'uploads/images/video/';
	var $path_dir_webp = 'uploads/webps/video/';
	var $control = 'video';
	...
	public function index()
	{
		...
		$datas = $this->VideoModels->select_array('id,name,thumb,created_at,publish,sort', NULL, 'sort asc,id desc');
		$data = [
	...
	function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max = $this->VideoModels->select_max('sort');
		if ($this->request->getPost()) {
			...
			$result = $this->VideoModels->add($data_post);
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas 		= $this->VideoModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->VideoModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$datas = $this->VideoModels->where_in(array('id' => $id));
		...
		if ($datas != NULL) {
			...
			$result = $this->VideoModels->deleteWhere(array('id' => $id));
		} else {
			$result = $this->VideoModels->deleteWhere(array('id' => $id));
		}
	}
	function deleteAll()
	{
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->VideoModels->where_in(array('id' => $val));
			$filename = $this->path_dir . $datas['image'];
			$filenamethumb = $this->path_dir . $datas['thumb'];
			// webp
			if ($datas != NULL) {
				...
				$result = $this->VideoModels->deleteWhere(array('id' => $val));
				if ($result) {
	...
	function checkglobals()
	{
		...
		$result = $this->VideoModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	public function sort()
	{
		...
		$result = $this->VideoModels->edit($data_update, array('id' => $id));
		if ($result > 0) {
