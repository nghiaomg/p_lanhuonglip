...
use App\Models\WhychooseModels;
...
{
	private $title 	  = 'Tại sao chọn';
	private $template = 'backend/whychoose/';
	var $control 	  = 'whychoose';
	public $path_dir = 'uploads/images/whychoose/';
	function __construct()
	{
		$this->WhychooseModels 			= new WhychooseModels();
	...
	public function index()
	{
		...
		{	
			$files = $this->path_dir . $config['image'];

			$files_thumb =  $this->path_dir . $config['thumb'];
			...
			if ($_FILES['image']['name']) {
				...
				$data_upload = $this->Upload->upload_image($path_dir, $fileName, uniqid(), 'image', 600, 600, null, null);
		...
		$data = [
			...
			'datas'		 =>	$this->WhychooseModels->select_array(),
	...
	public function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max   = $this->WhychooseModels->select_max('sort');
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
			if ($_FILES['image']['name']) {
				$fileName = $this->request->getFile('image');
			...
			$result = $this->WhychooseModels->add($data_post);
	...
	function checkglobals()
	{
		...
		$result 				 = $this->WhychooseModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas = $this->WhychooseModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->WhychooseModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
				return redirect()->to(base_url(CPANEL . $this->control . '/index'))->with('success', EDIT_SUCCESS);
			}
		}
	...
	function delete()
	{
		...
		$result = $this->WhychooseModels->deleteWhere(array('id' => $id));
	}
	function deleteAll()
	{
		//check login
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->WhychooseModels->find_one($val);
			$image = $this->path_dir . $datas['image'];
			$imagethumb = $this->path_dir . $datas['thumb'];
			try {
				if (file_exists($image)) {
			...
			$result = $this->WhychooseModels->deleteWhere(array('id' => $val));
			if ($result) {
	...
	public function sort()
    {
        ...
        $result              = $this->WhychooseModels->edit($data_update, array('id' => $id));
