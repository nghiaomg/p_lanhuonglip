...
{
	private $title 	  = 'Câu hỏi thường gặp';
	private $template = 'backend/faq/';
	var $control 	  = 'faq';
	public $path_dir = 'uploads/images/faq/';
	function __construct()
	{
		$this->FaqsModels 			= new FaqsModels();
	...
	public function index()
	{
		...
		$data = [
			...
			'datas'		 =>	$this->FaqsModels->select_array(),
	...
	public function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max   = $this->FaqsModels->select_max('sort');
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
			...
			$result = $this->FaqsModels->add($data_post);
	...
	function checkglobals()
	{
		...
		$result 				 = $this->FaqsModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	function edit($id)
	{
		...
		if ($this->request->getPost()) {
			...
			$result = $this->FaqsModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
				return redirect()->to(base_url(CPANEL . $this->control . '/index'))->with('success', EDIT_SUCCESS);
			}
		}
		...
		$data = [
			'data_index' =>	$data_index,
			'datas'		 =>	$this->FaqsModels->find_one($id),
			'control'	 =>	$this->control,
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$result = $this->FaqsModels->deleteWhere(array('id' => $id));
	}
	function deleteAll()
	{
		//check login
		...
		foreach ($list_id_delete as $key => $val) {
			$result = $this->FaqsModels->deleteWhere(array('id' => $val));
			if ($result) {
	...
	public function sort()
    {
        ...
        $result              = $this->FaqsModels->edit($data_update, array('id' => $id));
