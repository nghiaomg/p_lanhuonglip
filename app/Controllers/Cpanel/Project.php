...
{
	private $title 	  = 'Dự án';
	private $template = 'backend/project/';
	var $control 	  = 'project';
	public $path_dir = 'uploads/images/project/';
	function __construct()
	{
		$this->ProjectModels 			= new ProjectModels();
	...
	public function index()
	{
		...
		{	
			...
			if ($_FILES['image']['name']) {
				...
				$data_upload = $this->Upload->upload_image($path_dir, $fileName, $_FILES['image']['name'], 'image', 800, 800, null, null);
		...
		$data = [
			...
			'datas'		 =>	$this->ProjectModels->select_array(),
	...
	public function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max   = $this->ProjectModels->select_max('sort');
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
			if ($_FILES['image']['name']) {
				$fileName = $this->request->getFile('image');
				$path_dir = $this->path_dir;
				$data_upload = $this->Upload->upload_image($path_dir, $fileName, uniqid(), 'image', 800, 800, null, null);
			...
			$result = $this->ProjectModels->add($data_post);
	...
	function checkglobals()
	{
		...
		$result 				 = $this->ProjectModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas = $this->ProjectModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->ProjectModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
				return redirect()->to(base_url(CPANEL . $this->control . '/index'))->with('success', EDIT_SUCCESS);
			}
		}
	...
	function delete()
	{
		...
		$result = $this->ProjectModels->deleteWhere(array('id' => $id));
	}
	function deleteAll()
	{
		//check login
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->ProjectModels->find_one($val);
			$image = $this->path_dir . $datas['image'];
			$imagethumb = $this->path_dir . $datas['thumb'];
			try {
				if (file_exists($image)) {
			...
			$result = $this->ProjectModels->deleteWhere(array('id' => $val));
			if ($result) {
	...
	public function sort()
    {
        ...
        $result              = $this->ProjectModels->edit($data_update, array('id' => $id));
