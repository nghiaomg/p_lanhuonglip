...
class Productvideo extends BaseController
{
	private $title = 'video';
	private $template = 'backend/productvideo/';
	var	$path_url = 'cpanel/productvideo/';
	var $path_dir = 'uploads/images/productvideo/';
	var $path_dir_webp = 'uploads/webps/productvideo/';
	var $control = 'productvideo';
	...
	public function index($productID)
	{
		...
		$datas = $this->ProductvideoModels->select_array('*', ['productID' => $productID], 'sort asc,id desc');
	...
	function add($productID)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max = $this->ProductvideoModels->select_max('sort');
		if ($this->request->getPost()) {
			...
			$result = $this->ProductvideoModels->add($data_post);
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas 		= $this->ProductvideoModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->ProductvideoModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$datas = $this->ProductvideoModels->where_in(array('id' => $id));
		...
		if ($datas != NULL) {
			...
			$result = $this->ProductvideoModels->deleteWhere(array('id' => $id));
		} else {
			$result = $this->ProductvideoModels->deleteWhere(array('id' => $id));
		}
	}
	function deleteAll()
	{
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->ProductvideoModels->where_in(array('id' => $val));
			$filename = $this->path_dir . $datas['image'];
			$filenamethumb = $this->path_dir . $datas['thumb'];
			// webp
			if ($datas != NULL) {
				...
				$result = $this->ProductvideoModels->deleteWhere(array('id' => $val));
				if ($result) {
	...
	function checkglobals()
	{
		...
		$result = $this->ProductvideoModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	public function sort()
	{
		...
		$result = $this->ProductvideoModels->edit($data_update, array('id' => $id));
		if ($result > 0) {
