...
use App\Models\ProcedureModels;
...
{
	private $title 	  = 'Quy trình';
	private $template = 'backend/procedure/';
	var $control 	  = 'procedure';
	public $path_dir = 'uploads/images/procedure/';
	function __construct()
	{
		$this->ProcedureModels 			= new ProcedureModels();
	...
	public function index()
	{
		...
		$data = [
			...
			'datas'		 =>	$this->ProcedureModels->select_array(),
	...
	public function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max   = $this->ProcedureModels->select_max('sort');
		if ($this->request->getPost()) {
			$data_post = $this->request->getPost('data_post');
			...
			$result = $this->ProcedureModels->add($data_post);
	...
	function checkglobals()
	{
		...
		$result 				 = $this->ProcedureModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	function edit($id)
	{
		...
		if ($this->request->getPost()) {
			...
			$result = $this->ProcedureModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
				return redirect()->to(base_url(CPANEL . $this->control . '/index'))->with('success', EDIT_SUCCESS);
			}
		}
		...
		$data = [
			'data_index' =>	$data_index,
			'datas'		 =>	$this->ProcedureModels->find_one($id),
			'control'	 =>	$this->control,
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$result = $this->ProcedureModels->deleteWhere(array('id' => $id));
	}
	function deleteAll()
	{
		//check login
		...
		foreach ($list_id_delete as $key => $val) {
			$result = $this->ProcedureModels->deleteWhere(array('id' => $val));
			if ($result) {
	...
	public function sort()
    {
        ...
        $result              = $this->ProcedureModels->edit($data_update, array('id' => $id));
