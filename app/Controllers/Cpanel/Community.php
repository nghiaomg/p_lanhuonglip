...
{
	private $title = 'Community';
	private $template = 'backend/community/';
	var	$path_url = 'cpanel/community/';
	var $path_dir = 'uploads/images/community/';
	var $path_dir_webp = 'uploads/webps/community/';
	var $control = 'community';
	...
	public function index($productID)
	{
		...
		$datas = $this->CommunityModels->select_array('*', ['productID' => $productID], 'sort asc,id desc');
	...
	function add($productID)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max = $this->CommunityModels->select_max('sort');
		if ($this->request->getPost()) {
			...
			$result = $this->CommunityModels->add($data_post);
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas 		= $this->CommunityModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->CommunityModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$datas = $this->CommunityModels->where_in(array('id' => $id));
		...
		if ($datas != NULL) {
			...
			$result = $this->CommunityModels->deleteWhere(array('id' => $id));
		} else {
			$result = $this->CommunityModels->deleteWhere(array('id' => $id));
		}
	}
	function deleteAll()
	{
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->CommunityModels->where_in(array('id' => $val));
			$filename = $this->path_dir . $datas['image'];
			$filenamethumb = $this->path_dir . $datas['thumb'];
			// webp
			if ($datas != NULL) {
				...
				$result = $this->CommunityModels->deleteWhere(array('id' => $val));
				if ($result) {
	...
	function checkglobals()
	{
		...
		$result = $this->CommunityModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	public function sort()
	{
		...
		$result = $this->CommunityModels->edit($data_update, array('id' => $id));
		if ($result > 0) {
