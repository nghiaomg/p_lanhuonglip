...
class QuangCao extends BaseController
{
	private $title = 'Quảng cáo';
	private $template = 'backend/quangCao/';
	var	$path_url = 'cpanel/quangCao/';
	var $path_dir = 'uploads/images/quangcao/';
	var $path_dir_webp = 'uploads/webps/quangcao/';
	var $control = 'quangCao';
	...
	public function index()
	{
		...
		$datas = $this->QuangcaoModels->select_array('id,name,thumb,created_at,publish,sort', NULL, 'sort asc,id desc');
		$data = [
	...
	function add()
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$sort_max = $this->QuangcaoModels->select_max('sort');
		if ($this->request->getPost()) {
			...
			$result = $this->QuangcaoModels->add($data_post);
	...
	function edit($id)
	{
		...
		$this->checkPermission();
		$data_index = $this->get_indexBE();
		$datas 		= $this->QuangcaoModels->find_one($id);
		...
		if ($this->request->getPost()) {
			...
			$result = $this->QuangcaoModels->edit($data_post, array('id' => $id));
			if ($result['type'] == "successful") {
	...
	function delete()
	{
		...
		$this->checkPermission();
		$id = $_POST['id'];
		$datas = $this->QuangcaoModels->where_in(array('id' => $id));
		...
		if ($datas != NULL && $datas['image'] != NULL && $datas['thumb'] != NULL) {
			...
			$result = $this->QuangcaoModels->deleteWhere(array('id' => $id));
		} else {
			$alias = $this->AliasModels->deleteWhere(array('alias' => $datas['alias']));
			$result = $this->QuangcaoModels->deleteWhere(array('id' => $id));
		}
	...
	function deleteAll()
	{
		...
		foreach ($list_id_delete as $key => $val) {
			$datas = $this->QuangcaoModels->where_in(array('id' => $val));
			$filename = $this->path_dir . $datas['image'];
			...
			if ($datas != NULL) {
				...
				$alias = $this->AliasModels->deleteWhere(array('alias' => $datas['alias']));
				$result = $this->QuangcaoModels->deleteWhere(array('id' => $val));
				if ($result) {
	...
	function checkglobals()
	{
		...
		$result = $this->QuangcaoModels->edit($dataUpdate, array('id' => $id));
		if ($result) {
	...
	public function sort()
	{
		...
		$result = $this->QuangcaoModels->edit($data_update, array('id' => $id));
		if ($result > 0) {
