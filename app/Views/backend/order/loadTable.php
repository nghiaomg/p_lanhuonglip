<table class="table table-bordered  dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
    <thead class="thead-light">
        <tr>
            ...
            <th class="text-center nosort">Địa chỉ</th>
            <th class="text-center nosort">Ngày giao</th>
            <th class="text-center nosort">Giờ giao</th>
            <th class="text-center nosort">Tổng giá</th>
        ...
        <?php if (isset($datas) && $datas != null) { ?>
            <?php foreach ($datas as $key => $val) { ?>
                <tr class="tr<?= $val['id'] ?>">
                    ...
                    <td><?= $val['phone'] ?></td>
                    <td><?= $val['address'] ?></td>
                    <td><?= date('d/m/Y',strtotime($val['date_give']))?></td>
                    <td><?= $val['hours'] ?> giờ</td>
                    <td class="text-danger text-right" width="120"><?= number_format($val['total_price']) ?> đ</td>
                    <td><?= $val['created_at'] ?></td>
                    <td class="text-center">
                        <a href="javascript:void(0)" data-toggle="modal" onclick="view_detail(<?= $val['id'] ?>)" class="btn btn-primary">Chi tiết đơn hàng</a>
