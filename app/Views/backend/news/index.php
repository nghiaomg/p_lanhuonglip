			...
			<table id="datatable" class="table table-bordered  dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
				<thead class="thead-light">
					<tr>
						...
						<th class="text-center nosort">Hiển thị</th>
						<th class="text-center nosort">Home</th>
					...
					<?php if (isset($datas) && $datas != null) { ?>
						<?php foreach ($datas as $key => $val) { ?>
							<tr class="tr<?= $val['id'] ?>">
								...
								<td><?= $val['name'] ?></td>
								<td class="text-center"><?= $val['cate_name'] ?></td>
								...
								<td class="text-center">
									<input type="checkbox" onclick="changeProminent(<?= $val['id'] ?>,'home')" <?= $val['home'] == 1 ? 'checked' : '' ?> id="home<?= $val['id'] ?>" class="" data-control="<?= $control ?>">
								</td>
								<td class="text-center">
