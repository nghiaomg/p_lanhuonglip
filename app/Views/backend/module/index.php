			...
			<table id="datatable" class="table table-bordered  dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
				<thead class="thead-light">
					<tr>
						...
						<th>Tên</th>
						<th class="text-center nosort">Icon</th>
					...
					<?php if (isset($datas) && $datas != null) { ?>
						<?php foreach ($datas as $key => $val) { ?>
							<tr class="tr<?= $val['id'] ?>">
								...
								<td><?= $val['name'] ?></td>
								<td class="text-center"><i class="<?= $val['icon'] ?> text-info"></i></td>
							...
							<?php if (isset($val['child']) && $val['child'] != NULL) { ?>
								<?php foreach ($val['child'] as $key_child => $val_child) { ?>
									<tr class="tr<?= $val_child['id'] ?>">
										<td class="text-center"><input type="checkbox" name="foo" class="checkbox" value="<?= $val_child['id'] ?>"></td>
										<td> -------------- <?= $val_child['name'] ?></td>
										<td class="text-center"><i class="<?= $val['icon'] ?> text-info"></i></td>
										<td class="text-center">
											<input type="checkbox" onclick="changeglobal(<?= $val_child['id'] ?>,'publish')" <?= $val_child['publish'] == 1 ? 'checked' : '' ?> id="publish<?= $val_child['id'] ?>" class="" data-control="<?= $control ?>">
										</td>
										<td class="text-center">
											<input type="text" onkeyup="changeSort(this,<?= $val_child['id'] ?>)" value="<?= $val_child['sort'] ?>" class="text-center form-control publish" id="sort<?= $val_child['id'] ?>" class="" data-control="<?= $control ?>">
										</td>
										<td class="text-center"><?= date('d/m/Y', strtotime($val_child['created_at'])) ?></td>
										<td class="text-center">
											<a href="javascript:void(0)" onclick="del(<?= $val_child['id'] ?>)" class="btn btn-danger btn-sm" id="delete<?= $val_child['id'] ?>" data-control="<?= $control ?>"><i class="icon-trash"></i></a>
											<a href="<?= base_url(CPANEL . $control . '/edit') . '/' . $val_child['id'] ?>" class="btn btn-info btn-sm"><i class="icon-note"></i></a>
										</td>
									</tr>
								<?php } ?>
							<?php } ?>
